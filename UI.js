...
function setDDMs() {
    ...
    for(var g = 0; g < DDMs.length; g++) {
        var DDM = DDMs[g];
        DDM.id = "UI_DROP_" + ++NumOfDDMs;
        var value = DDM.innerHTML;
        DDM.innerHTML = "";
        var sp1 = ce("span");
        sp1.id = DDM.id + "_CONTENT";
...
function UIDrop(DDB) {
    ...
    for(var g = 0; g < items.length; g++) {
        ...
        item.textContent = items[g];
        
        if(items[g] == DDB.textContent) item.style.fontWeight = "bold";
        item.onclick = function(e) {
            var DDB = this.parentNode.parentNode;
            document.getElementById(DDB.id + "_CONTENT").textContent = this.textContent;
            removeUIDrop();
            try {DDB.onchange();} catch(e) {}
            e.stopPropagation();
        }
